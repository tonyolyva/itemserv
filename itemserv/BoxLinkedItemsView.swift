...
struct BoxLinkedItemsView: View {
    ...
    private var sortedBoxNames: [BoxName] {
        ...
        let paired = numericBoxes.map { box in
            let number = Int(box.boxNameText) ?? Int.max - (box.boxNameText.hashValue % 1000)
            return (box, number)
        }
    ...
    private var virtualUnboxedItems: [Item] {
        boxNames.flatMap { ($0.items as? [Item])?.filter { $0.boxNameRef?.boxNameText == "Unboxed" } ?? [] }
    }
        ...
        ZStack {
            VStack {
                ...
                ScrollViewReader { proxy in
                    ...
                    .onChange(of: scrollToBoxID) { id in
                        if let id = id {
                            withAnimation {
    ...
    func addBoxName() {
        withAnimation {
            ...
            let existingNumbers = boxNames.compactMap { Int($0.boxNameText) }.sorted()
            ...
            // 3. Prevent duplicates (safety check)
            guard !boxNames.contains(where: { $0.boxNameText == newBoxNameText }) else { return }
